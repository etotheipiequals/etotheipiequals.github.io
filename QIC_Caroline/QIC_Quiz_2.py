...
p.newQuiz(#(a) and (c)
    ...
    answersText=[
        ...
        r"$({\rm b})$ Photon polarization. As there are two states of polarization we can use a qubit state vector to represent polarization. For light polarization, the Bloch sphere is usually called the Poincare sphere.",
        r"$({\rm c})$ Quantum entanglement. $[{\rm CORRECT}]$. Entanglement needs more than one qubit. We CANNOT represent entanglement using individual Bloch spheres. ",
        r"$({\rm d})$ Decoherence. In the Bloch sphere picture decoherence can be represented as a decay of length of the Bloch vector towards the $z$ axis.",
        r"$({\rm e})$ Any two-level system.",
        r"$({\rm f})$ An electron spin.",
    ],
)
...
p.newQuiz( # (c) (d) and (e)
    questionImage="./QIC_Figures/QIC2_Bloch_2.png",fontSize=0.5,
    questionText=r"$9.$ Which of the following are possible qubit states?  $[{\rm SOLUTION}]:$ Check if $\vert a\vert ^2 + \vert b\vert^2 =1$.",
    answersText=[
...
p.newQuiz(# (c) and (f)
    ...
    answersText=[r"$({\rm a})$ \begin{eqnarray}\vert \psi \rangle * \vert \psi \rangle \end{eqnarray} is a $6\times1$ matrix. $[{\rm NO}]$ this is $9\times1$ (9 rows, 1 column, i.e. a column vector)",
                 r"$({\rm b})$ \begin{eqnarray} \langle  \psi \vert * \langle  \psi \vert \end{eqnarray} is a $9\times1$ matrix. $[{\rm NO}]$",
                 r"$({\rm c})$ \begin{eqnarray}\langle  \psi \vert * \langle  \psi \vert \end{eqnarray} is a $1\times9$ matrix. $[{\rm YES}]$ one row, nine columns, a row vector.",
                 r"$({\rm d})$ \begin{eqnarray}\langle \psi\vert \psi \rangle \end{eqnarray} is a $9\times9$ matrix. $[{\rm NO}]$ this is a number, $1\times1$. ",
                 r"$({\rm e})$ \begin{eqnarray}\vert \psi \rangle \langle \psi\vert   \end{eqnarray} is a $9\times9$ matrix. $[{\rm NO}]$ that would be true for two qutrits but there is only one.",

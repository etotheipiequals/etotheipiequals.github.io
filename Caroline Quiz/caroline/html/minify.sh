...
if [ "$deploy" = true ]; then
  ...
  SCRIPT="
  sudo rsync -avz ~/transfer_roundtable/* /usr/share/nginx/html2/.
  sudo chown root:root -R /usr/share/nginx/html2/*
  exit"

  sudo ssh -i ~/Documents/Desktop/Pet_Projects/researchx3d/deployment/rx3d-ec2-front-server.pem ec2-user@www.researchx3d.com "${SCRIPT}"

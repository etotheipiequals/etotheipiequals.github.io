...
class Presentation:
    def __init__(
        ...
        self.style = []
        self.logo = logo
        self.leftPartExists = False  # if left part of the slide is filled
        self.leftHanded = leftHanded  # if presenter is left handed, move menu right
        self.grid = False  # do we currently construct grid layout slide
    ...
    def _textBelowAboveHeight(self, textBelow, textAbove, height, width=None):
        ...
        if height is not None:
            style += "height:%spx;max-height:100%%;" % height
        if width is not None:
            style += "width:%spx;max-width:100%%;" % width
        if style != "":
            style = "style='" + style + "'"

...
from flask_socketio import disconnect, SocketIO, emit, join_room, leave_room, ConnectionRefusedError
import requests
import jwt
import json
import qrcode
...
app = Flask(__name__)
CORS(app)
app.config['SECRET_KEY'] = 'aogrnAWUrf32pgwnr983nba%7asfq9*'
socketio = SocketIO(app,  cors_allowed_origins="*")
apiPrefix = "/api"
subscriberKey = "osagASfew8t31qNqfHQ3Gq"
...
def presentationQRcode():
    ...
    qr = qrcode.QRCode()
    qr.add_data(request.args.get('q')
...
def test_connection():
    ...
    if ("pkey" in request.args and "site" in request.args):
        if (request.args.get('pkey') == subscriberKey):
            payload = {
                'exp': datetime.datetime.utcnow() + datetime.timedelta(days=0, seconds=2*60*60),
                'site': request.args.get('site')
            }
            emit('initial_response',
    ...
    if ("auth" in request.args):
        r = requests.post("https://www.google.com/recaptcha/api/siteverify",
                          data={'secret': '6LdpyLIUAAAAAHy34n-QVAUuSCNGeisuB98W1Nyr',
        ...
        try:
            payload = jwt.decode(request.args.get('auth2'),
                                 app.config.get('SECRET_KEY'),
                                 algorithms=["HS256"])
            if ( datetime.datetime.utcfromtimestamp(payload["exp"])
...
def on_join(data):
    ...
    room = data['room']
    join_room(room)
    emit("new_user",{"username":username, "sid":request.sid},
...
def send_to_room(message):
    ...
    emit('update_state',
         message,
         broadcast=True, to=message["room"], include_self=False)
    return timestamp
...
def mouse_move(message):
    emit('m',
         message,
         broadcast=True, to=message["r"], include_self=False)
...
def send_to_room(message):
    ...
    emit('update_state',
         message,
         broadcast=True, to=message["room"])
...
def send_to_user(message):
    ...
    emit('update_state',
         message,
         broadcast=True, to=message["sid"], include_self=False)
